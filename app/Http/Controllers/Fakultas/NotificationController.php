...
class NotificationController extends Controller
{
    ...
    public function read_notification (Request $request)
    {
        ...
        if($getById->to_role != 4) abort(404);
        if($getById->to_id != auth()->user()->fakultas) abort(404);

        $jenis = $getById->jenis_notifikasi;
        $getById->update(['read_status' => 'read']);
        if ($jenis == 'Submission') {
            $sub = \App\Models\Submission::find($getById->id_jenis);
            return redirect()->to(route('fakultas.usulan-baru.show', ['type' => $sub->tipe_submission, 'submission_code' => $sub->submission_code]));
        }
    }
}
