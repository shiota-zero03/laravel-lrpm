...
class MonevController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = Submission::join('users', 'users.id', '=', 'submissions.id_pengaju')
                            ...
                            ->whereNot('submissions.status_monev', null)
                            ->select('submissions.*', 'users.fakultas')
                            ->orderByDesc('submissions.id')
                            ->get();
        return view('pages.fakultas.submission.monev.index', compact(['type', 'data']));
    }
    public function tracking(Request $request)
    {
        ...
        return view('pages.fakultas.submission.monev.tracking', compact(['type', 'data', 'timeline']));
    }
    public function show(Request $request)
    {
        ...
        return view('pages.fakultas.submission.monev.show', compact(['type', 'check_code', 'user', 'nilai']));
    }
    public function action(Request $request)
    {
        ...
        if($request->aksi == 'Revised') {
            ...
            if(strtotime($request->waktu_sidang) < strtotime('+1 week')){
                return response()->json(['message' =>'Waktu sidang harus minimal satu minggu dari sekarang'], 403);
            } else {
                $update = $check_code->update([
