...
class LaporanAkhirController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data = Submission::where('tipe_submission', $type)
                            ...
                            ->orderByDesc('id')
                            ->get();
        return view('pages.reviewer.submission.laporan-akhir.index', compact(['type', 'data']));
    }
    ...
    public function show(Request $request)
    {
        ...
        return view('pages.reviewer.submission.laporan-akhir.show', compact(['type', 'check_code', 'user', 'select']));
    }
        ...
        if($request->aksi == 'fti') {
            if(isset($_POST['store'])){
                \App\Models\Notification::create([
                    ...
                    'text_notifikasi' => 'Laporan akhir dengan nomor pengajuan '.$check_code->submission_code.' telah dinilai, menunggu verifikasi oleh admin',
                    'to_role' => 5,
                ...
                \App\Models\Notification::create([
                    ...
                    'jenis_notifikasi' => 'Laporan',
                    'judul_notifikasi' => 'Informasi penerimaan laporan',
                    'text_notifikasi' => 'Laporan akhir dengan nomor pengajuan '.$check_code->submission_code.' telah dinilai kembali, menunggu verifikasi oleh admin',
                    'to_role' => 1,
                    'read_status' => 'unread',
                ]);
                \App\Models\TimelineProgress::create([
            ...
            $update = $check_code->update([
                'status_laporan_akhir' => 'Waiting for Validation',
            ]);

            $check_submission = \App\Models\FtiEvaluation::where('id_submission', $check_code->id)->where('tipe', 'Laporan Akhir')->get()->count();

            if($check_submission > 0) {
                $get_submission = \App\Models\FtiEvaluation::where('id_submission', $check_code->id)->where('tipe', 'Laporan Akhir')->first();
                $create = \App\Models\FtiEvaluation::find($get_submission->id)->update($data);
            } else {
                $create = \App\Models\FtiEvaluation::create($data);
            }
            if($create) {
                return redirect()->to(route('reviewer.laporan-akhir.index', ['type' => $request->type]));
            }
            ...
            $check_submission = \App\Models\FbisEvaluation::where('id_submission', $check_code->id)->where('tipe', 'Laporan Akhir')->get()->count();

            if($check_submission > 0) {
                $get_submission = \App\Models\FbisEvaluation::where('id_submission', $check_code->id)->where('tipe', 'Laporan Akhir')->first();
