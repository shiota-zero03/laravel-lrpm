            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between pt-4">
                        <a href="{{ route('admin.spk.index', ['type' => $type]) }}" class="btn btn-info text-white"><i class="bi bi-arrow-left me-2"></i>Kembali</a>
                        <a href="{{ route('admin.spk.tracking', ['type' => $type, 'id' => $check_code->id]) }}" class="btn btn-primary">Lihat traking</a>
                    </div>
                    <div>
                        ...
                        <div class="row">
                            <div class="col-12 mb-2">
                                ...
                                @if( $check_code->spk_upload )
                                    <a href="{{ asset('assets/storage/files/spk-upload/'.$check_code->spk_upload) }}" class="btn btn-success w-100" id="file-usulan">
                                        <i class="bi bi-cloud-arrow-down-fill me-2"></i>Lihat dan Download SPK
                                    </a>
                                @else
                                    <div class="form-control border border-danger text-danger fw-bold fst-italic text-uppercase text-center">
                                        Belum ada SPK
                        ...
                        @if($check_code->status_spk == 'Pending by Admin')
                            <form action="aksi/{{$check_code->id}}/Approved" method="post" enctype="multipart/form-data">
                                @csrf
                                <div class="file_upload mb-3">
                                    <label for="file_upload">Upload file spk yang telah ditandatangani</label>
                                    <input required type="file" name="file_upload" class="form-control" id="file_upload">
                                </div>
                                <div class="d-md-flex">
                                    <button type="submit" class="btn btn-primary text-white me-2 mb-2"><small>Setujui</small></button>
                                    <a onclick="action()" class="btn btn-warning me-2 mb-2 text-white"><small>Kembalikan</small></a>
                                </div>
                            </form>
                        @endif
                    </div>
                                ...
                                $.ajax({
                                    ...
                                    success: function(response){
                                        ...
                                        Swal.fire({
                                            ...
                                            document.location.href="{{ route('admin.spk.index', $check_code->tipe_submission) }}"
                                        });
                                    },
