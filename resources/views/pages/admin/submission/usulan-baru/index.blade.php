                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                ...
                                                @endif
                                            @endif
                                            @if( $usulan->status_usulan == 'Waiting for Validation' )
                                                @if( $usulan->second_status == 'Approved' )
                                                    <small><strong><em>Data disetujui prodi</em></strong></small>
                                                @else
                                                    <small>
                                                        Alasan @if( $usulan->second_status == 'Rejected' ) ditolak @else dikembalikan @endif: <b>{{ $usulan->alasan_usulan_ditolak }}</b>
                                                    </small>
                                                @endif
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{route('admin.usulan-baru.show', ['type' => $usulan->tipe_submission, 'submission_code' => $usulan->submission_code])}}" class="btn text-white btn-primary mb-1 me-1 btn-sm" data-bs-toggle="tooltip" title="Detail"><strong><i class="bi bi-eye"></i></strong></a>
                                            <a href="{{route('admin.usulan-baru.tracking', ['type' => $usulan->tipe_submission, 'id' => $usulan->id])}}" class="btn btn-info mb-1 me-1 text-white btn-sm" data-bs-toggle="tooltip" title="Traking"><i class="bi bi-search"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
