            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex justify-content-between pt-4">
                        <a href="{{ route('admin.laporan-final.index', ['type' => $type]) }}" class="btn btn-info text-white"><i class="bi bi-arrow-left me-2"></i>Kembali</a>
                        <a href="{{ route('admin.laporan-final.tracking', ['type' => $type, 'id' => $check_code->id]) }}" class="btn btn-primary">Lihat traking</a>
                    </div>
                    <div>
                        ...
                        <div class="row">
                            <div class="col-12 mb-2">
                                ...
                                @else
                                    <a href="{{ asset('assets/storage/files/laporan-final/'.$check_code->laporan_akhir) }}" class="btn btn-success w-100" id="download-usulan">
                        ...
                        @endif
                        @if($check_code->status_laporan_final == 'Pending by Admin')
                            <div class="d-md-flex">
                        ...
                        @endif
                        @if($check_code->status_laporan_final == 'Waiting for Validation')
                            <div class="d-md-flex">
                                <button type="button" onclick="action('to_reviewer')" class="btn btn-warning text-white me-2 mb-2"><small>Kembalikan ke Reviewer</small></button>
                                <button type="button" onclick="action('setujui')" class="btn btn-primary me-2 mb-2 text-white"><small>Setujui Penilaian</small></button>
                            </div>
                        @endif
                    </div>
                </div>
...
@push('custom_script')
    ...
    <script>
        ...
        function action(action){
            Swal.fire({
                text: 'Anda yakin ingin '+(action === 'to_reviewer' ? ('mengembalikan data ini ke reviewer ?') : (action+' data ini ?')),
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    if(action === 'mengembalikan'){
                        Swal.fire({
                            text: "Tulis alasan pengembalian pengajuan",
                            input: "text",
                            showCancelButton: true,
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response){
                                            ...
                                            Swal.fire({
                                                ...
                                                document.location.href="{{ route('admin.laporan-final.index', $check_code->tipe_submission) }}"
                                            });
                                        },
