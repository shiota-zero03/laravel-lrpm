...
var table = $('#skema-datatable').DataTable({
    processing: true,
    serverSide: true,
    ajax: "skema/json",
    columns: [
        {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
        {data: 'nama_skema', name: 'nama_skema'},
    ...
    if(id !== 0) {
        $.ajax({
            ...
            success: function(response){
                ...
                $('form').find('input[name=nama]').val(response.data.nama_skema);
...
$('#btn_save').on('click', function(){
    ...
    const formData = {
        ...
        nama_skema: $('form').find('input[name=nama]').val(),
    }
        ...
        $.ajax({
            ...
            error: function(error){
                if(error.status === 422) {
                    ...
                    if(errorResponse['nama_skema']) $('#error_nama').html(errorResponse['nama_skema'][0])
                }
                $('#loading-submit').addClass('d-none')
            }
        })
